...
이 조건에 맞는, 지금 소싱하기 좋은 **카테고리(상품군)** 10~12개를 최대한 다양하게(서로 겹치지 않는 여러 생활 영역에 걸쳐서) 추천해주세요. 시장 규모가 클 것 같은 것부터 작고 틈새인 것까지, 경쟁이 치열할 것 같은 것부터 아직 안 붐빌 것 같은 것까지 골고루 섞어주세요 - 나중에 실데이터로 검증한 뒤 시장규모/경쟁강도 조합별로 걸러서 보여줄 예정이라, 후보 단계에서부터 스펙트럼이 다양해야 합니다. "패션의류" 같은 너무 넓은 대분류 말고, "여름 휴대용 냉방 소품" "캠핑 조명용품" 처럼 실제로 무엇을 찾아야 할지 감이 오는 수준의 구체성으로 적어주세요. 단, "A/B", "A·B", "A 및 B" 처럼 서로 다른 상품 여러 개를 하나로 묶은 이름은 절대 쓰지 마세요 — 하나의 일관된 상품군이어야 합니다 (예: "수건걸이/샴푸디스펜서 세트" (X), "선풍기/가습기" (X) → "욕실 정리용품" (O)).
