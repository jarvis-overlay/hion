...
export default function NaverCard({ connected }: { connected: boolean }) {
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-display font-bold">네이버</h3>
        <span
      ...
      {!editing ? (
        <div className="flex gap-2">
          ...
          <button
            onClick={() => startTransition(() => disconnectChannel('naver'))}
            disabled={isPending}
      ...
      <p className="text-[11px] text-inkSoft mt-3">
        네이버 커머스API센터(apicenter.commerce.naver.com) → 애플리케이션
        등록에서 발급받은 값을 입력해줘.
      </p>
    </div>
  );
